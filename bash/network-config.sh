...
myhostname=$(hostname) 
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #for Find interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #to find  a lan address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') # for find  lan hostname with getent hosts and variable  $lan_address
external_ip=$(curl -s icanhazip.com) #obtaining external IP address from curl
external_name=$(getent hosts $external_ip| awk '{print $2}') #Getting external ip by looking into hosts file by $external_ip
router_address=$(ip r s default| cut -d ' ' -f 3) #searching  a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}') #finding router hostname using getent hoste and $router_address
