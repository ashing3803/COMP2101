...
myargs=()
#Making variables to identify the stat of the input
vmodefverbose=0
dmodefverdebug=0
...
while [ $# -gt 0 ]; do
  ...
  *)
  ...
  #Shifting the command line argument to process the element
  shift
...
# to check stat of the variable applying if condition  

if [ $vmodefverbose = 1 ]; then
...
if [ $dmodefverdebug -gt 0 ]; then
  echo "Debug Mode is On with debug level $dmodefverdebug."
else
