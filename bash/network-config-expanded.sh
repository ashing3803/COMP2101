...
intlist=$(lshw -class network |  awk '/logical name:/{print $3}')
declare -a interface=($intlist)
len=${#interface[*]}
i=0
#Making a loop for processing each work in the array
while [ $i -lt $len ]
do
    if [[ ${interface[$i]} = lo* ]] ;
		then continue ;
	fi
	#Implimanting  the conditions to skip loopback interface
	  ...
	  #Normal lines for putting command's output to the variable
